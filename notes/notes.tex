...
\begin{paracol}{2}
	\question{Is \textit{abc} a Python string?}
	\note{Nope. Python strings are enclosed in single quotes (`...') \textit{or} double quotes (``...'') with the same result. Hence, \textit{``abc''} can be Python string, while \textit{abc} cannot. Example 5.3 (line 2) shows how to assign the string ``Python 3.X'' to the variable $S$.}
\end{paracol}
