...
>>> TO_WRITE = "/n".join([FIRTSTLAW, SECONDLAW, THIRDLAW])
...
\begin{table}[!htbp]
	...
	\begin{tabular}{ll}
		...
		Method       & Description                                                                          \\
		\midrule
		\texttt{close()     } & Closes the file                                                                      \\
		\texttt{detach()    } & Returns the separated raw stream from the buffer                                     \\
		\texttt{fileno()    } & Returns a number that represents the stream as per the OS' perspective \\
		\texttt{flush()     } & Flushes the internal buffer                                                          \\
		\texttt{isatty()    } & Returns whether the file stream is interactive or not                                \\
		\texttt{read()      } & Returns the file content                                                             \\
		\texttt{readable()  } & Returns whether the file stream can be read or not                                   \\
		\texttt{readline()  } & Returns one line from the file                                                       \\
		\texttt{readlines() } & Returns a list of lines from the file                                                \\
		\texttt{seek()      } & Change the file position                                                             \\
		\texttt{seekable()  } & Returns whether the file allows us to change the file position                       \\
		\texttt{tell()      } & Returns the current file position                                                    \\
		\texttt{truncate()  } & Resizes the file to a specified size                                                 \\
		\texttt{writable()  } & Returns whether the file can be written to or not                                    \\
		\texttt{write()     } & Writes the specified string to the file                                              \\
		\texttt{writelines()} & Writes a list of strings to the file                                                 \\
		\bottomrule
	\end{tabular}
\end{table}
